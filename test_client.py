...
class TestClient(TestCase):
    ...
    def test_asd_connection3(self):
        clients = [ThreadedClient("Client---->" + str(1)), ThreadedClient("Client---->" + str(2))]
        for client in clients:
            client.start()
            time.sleep(1)
        for client in clients:
...
class ThreadedClient(threading.Thread, sockethandler.CommonSocketHandler):
    ...
    def run(self):
        ...
        if isinstance(message, messages.RSPOK):
            ...
            while True:
                message = self.receive_msg()
                if isinstance(message, messages.RSPFIRST):
                    message2 = messages.MOVE()
                    message2.body.move = ((8, 4), (6, 4))
                    self.send_message(message2)
                    print str(message2)
                elif isinstance(message, messages.RSPSECOND):
